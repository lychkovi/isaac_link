...
int main(int argc, char **argv)
{
   ...
   */
  ros::init(argc, argv, "elbrus_sim");
  ROS_INFO("Starting elbrus_sim node...");
  ...
  ros::Rate loop_rate(10); // Отправка сообщения будет происходит с частотой 10 Гц
  ...
  while (ros::ok())
  {
    ...
    if (receiver.GetDataDummy(origin, rotation, allpts))
    {
        // Получено новое облако точек - публикуем его в ROS
